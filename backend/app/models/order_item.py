...
class OrderItem(Base):
    ...
    # Snapshot Attributes
    menu_name_snapshot = Column(String(100), nullable=False, comment="주문 시점 메뉴")
    menu_price_snapshot = Column(Float, nullable=False, comment="주문 시점 기본 가격")
    selected_options = Column(JSON, nullable=True, comment="선택 옵션 JSON")
    
    # Attributes
    quantity = Column(Integer, nullable=False, comment="수량")
    subtotal = Column(Float, nullable=False, comment="총 가격")
